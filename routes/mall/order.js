...
router.post('/settle', async function (req, res) {
    ...
    let [results] = await pool.query(goods_sql, [id, goods]);
    // 成功
    res.json({
        status: true,
        msg: "获取成功!",
...
router.post('/create', async function (req, res) {
    ...
    try {
        ...
        let [{ insertId, affectedRows: insert_affected_rows }] = await connection.query(insert_sql, [id, payment]);
        ...
        // 3.存储收货地址快照
        let copy_sql = `INSERT INTO order_address ( order_id, name, tel, province_id, city_id, county_id, town_id, street, code ) SELECT ( ? ), name, tel, province_id, city_id, county_id, town_id, street, code FROM address WHERE id = ?`;
        ...
        let [{ affectedRows: cut_affected_rows }] = await connection.query(cut_sql, [insertId, id, queryGid]);
        ...
        let [{ affectedRows: delete_affected_rows }] = await connection.query(delete_sql, [id, queryGid]);
...
router.get('/list', async function (req, res) {
    ...
    let [orders] = await pool.query(order_sql, [id, size, offset]);
    // 查询订单总数
    ...
    let [goods] = await pool.query(goods_sql, [id, size, offset]);
    // 循环遍历给订单数组添加订单商品信息
